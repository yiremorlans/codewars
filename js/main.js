...
function betterThanAverage(classPoints, yourPoints) {
  ...
  totalScore =
    totalScore.reduce((total, x) => total + x, 0) / totalScore.length;
...
Array.prototype.filter = function (fn) {
  ...
  for (var i = 0; i < this.length; i++) {
    //this keyword to reference param passed from fn
    if (fn(this[i])) {
      res.push(this[i]);
    }
  }
...
function all(arr, fun) {
  //function contains the test that will run on the elements.
...
// Implement a stack class without prototype methods
class Stack {
  constructor() {
    this.storage = {};
    this.size = 0;
