...
function solution(number) {
  ...
  return newArr
    .filter(
      (number) => number % 3 === 0 || number % 5 === 0 || number % 15 === 0
    )
...
function findOdd(A) {
  ...
  for (const [key, value] of Object.entries(objHoldingNums)) {
    //deconstructuring params to access both key/value
    if (value % 2 !== 0) {
...
function digital_root(n) {
  let reducedValue = String(n)
    .split("")
    .reduce((total, value) => +total + +value, 0);
  while (reducedValue.toString().length > 1) {
    reducedValue = String(reducedValue)
      .split("")
      .reduce((total, value) => +total + +value, 0);
  }
  return reducedValue;
...
function findOutlier(integers) {
  ...
  integers.filter((value) =>
    value % 2 === 0 ? evenNums.push(value) : oddNums.push(value)
  );
...
function isValidWalk(walk) {
  if (walk.length === 10) {
    return (
      walk.join("").split("n").length === walk.join("").split("s").length &&
...
function duplicateEncode(word) {
  ...
  for (let i = 0; i < word.length; i++) {
    if (!map[word[i].toLowerCase()]) {
      //ensures prop is written to hashmap as lowercase
  ...
  for (let i = 0; i < word.length; i++) {
    //writes encoding thru str.length
...
function titleCase(title, minorWords) {
  ...
  if (minorWords === undefined) {
    return title
      .toLowerCase()
      .split(" ")
      .map((value) => value.split("")[0].toUpperCase() + value.slice(1))
      .join(" ");
...
function alphabetPosition(text) {
  ...
  let sanitizedText = text
    .toLowerCase()
    .split("")
    .filter((value) => alphabetStr.includes(value))
    .join("");
  let resultStr = "";
...
function solution(string) {
  ...
  for (let i = 0; i < string.length; i++) {
    if (string[i] === string[i].toUpperCase()) {
      newStr +=
        `,${string[i]}` +
        string.slice(i, string[i] === string[i].toUpperCase());
    } else {
      newStr += string[i];
    }
  }
...
function high(x) {
  ...
  for (let i = 0; i < strArr.length; i++) {
    ...
    let score = Object.values(mapped).reduce(
...
function dup(s) {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    s[i]
      .split("")
      .forEach((value) =>
        newStr[newStr.length - 1] === value ? value : (newStr += value)
      );
...
function dup(s) {
  return s.map((value) =>
    value
      .split("")
      .filter((currentVal, index, arr) => currentVal !== arr[index + 1])
      .join("")
  );
}
...
decodeMorse = function (morseCode) {
  ...
  morseArr = morseArr.map((value, index, arr) =>
    value === arr[index + 1] && value === " " ? "" : value
  );
  return morseArr.join("");
};

// Tribonacci Sequence. Write a function that returns an array the length of n by conditions of fibonacci sequence but 3 instead of 2.
function tribonacci(signature, n) {
  if (n === 0) return [];
  if (n < 3) return signature.slice(0, n);

  let copyArr = signature.slice();

  for (let i = 0; i < n - signature.length; i++) {
    let reducedVal = copyArr
      .slice(copyArr.length - 3)
      .reduce((total, value) => total + value, 0);
    copyArr.push(reducedVal);
  }
