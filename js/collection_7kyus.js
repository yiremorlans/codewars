...
function getSum(...values) {
  //turned params (a,b) into (...values) to include use Math.min/max in for loop
  let array = [];
...
function openOrSenior(data) {
  return data.map(function ([age, handicap]) {
    //use destructuring assignment to unpack each variable in each pair of array
...
function numberOfPairs(gloves) {
  ...
  for (let i = 0; i < gloves.length; i++) {
    if (!objHoldingGloves[gloves[i]]) {
      //checks if objHoldingGloves key exists
    ...
    if (objHoldingGloves[gloves[i]] === 2) {
      //if glove color pair exists
...
function sumDigits(number) {
  ...
  let stringNumToArr = String(absole)
    .split("")
    .map((value) => +value);
...
function printerError(s) {
  ...
  let errorMarks = s
    .split("")
    .filter((letter) => !printerLetters.includes(letter));
...
function rowWeights(array) {
  ...
  return [
    teamOne.reduce((total, value) => total + value, 0),
    teamTwo.reduce((total, value) => total + value, 0),
  ];
}
...
function balancedNum(number) {
  ...
  return head.split("").reduce((total, current) => total + +current, 0) ===
    tail.split("").reduce((total, current) => total + +current, 0)
    ? "Balanced"
    : "Not Balanced";
}
...
function repeats(array) {
  return array
    .filter((value) => array.indexOf(value) === array.lastIndexOf(value))
...
function averageLength(arr) {
  let arrAvg = Math.round(
    arr.reduce((total, value) => total + value.length, 0) / arr.length
  );
...
// Validate digits in a pin input
function validatePIN(pin) {
  if (pin.length === 4 || pin.length === 6) {
    return pin.split("").every((num) => parseInt(num) >= 0);
  }
  return false;
}
